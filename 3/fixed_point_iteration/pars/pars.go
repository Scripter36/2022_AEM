...
func Make_list(formular string) []Querry {
	...
	// Parenthesis
	reg_exp = regexp.MustCompile("\\(")
	parenthel := reg_exp.FindAllStringSubmatchIndex(formular, -1)
	for _, ploc := range parenthel {
		Q.number, Q.identity, Q.name, Q.value, Q.operator =
	...
	reg_exp = regexp.MustCompile("\\)")
	parenther := reg_exp.FindAllStringSubmatchIndex(formular, -1)
	for _, qloc := range parenther {
		Q.number, Q.identity, Q.name, Q.value, Q.operator =
			qloc[0], "Q", "L", 0.0, string(formular[qloc[0]:qloc[1]])
		LIST = append(LIST, Q)
	}
